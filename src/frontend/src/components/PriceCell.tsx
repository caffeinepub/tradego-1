...
export function PriceCell({ instrument, livePrice, showChange = true, compact = false }: PriceCellProps) {
  const price = livePrice?.price ?? instrument.currentPrice;
  const prevPrice = livePrice?.prevPrice ?? instrument.previousClose;
  ...
  if (compact) {
    return (
      <div className="flex items-center gap-1.5">
        <span className={`font-mono text-sm font-semibold ${flashClass} rounded px-0.5 transition-colors`}>
  ...
  return (
    <div className="text-right">
      <div className={`font-mono text-sm font-semibold ${flashClass} rounded px-0.5 transition-colors ${direction === "up" ? "text-gain" : direction === "down" ? "text-loss" : "text-foreground"}`}>
        {formatPrice(price, instrument.category)}
      </div>
      {showChange && (
