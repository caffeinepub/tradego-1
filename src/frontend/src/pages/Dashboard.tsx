...
export function Dashboard({
  ...
  const portfolioCards = [
    ...
    },
    {
      ...
      colorClass:
        portfolio && portfolio.totalPnL >= 0 ? "text-gain" : "text-loss",
      bgClass:
    ...
    <div className="p-4 space-y-5 max-w-5xl mx-auto">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {portfolioCards.map(
            ({ label, value, icon: Icon, colorClass, bgClass }, idx) => (
              <Card
                key={label}
              ...
              >
                <CardContent className="p-3">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <div
                      className={`w-7 h-7 rounded-md ${bgClass} flex items-center justify-center shrink-0`}
                    >
          ...
          <Card className="bg-card border-border">
            <CardContent className="p-8 text-center">
              ...
              <p className="text-xs text-muted-foreground mt-1 mb-3">
                Add instruments from the Markets tab to track them here
              </p>
              <Button
                size="sm"
                variant="outline"
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      <div>
                        <div className="flex items-center gap-1.5">
                          ...
                          <Badge
                            variant="outline"
                            className="text-[9px] border-border text-muted-foreground capitalize px-1 py-0 h-4"
                          >
                            {instrument.category}
                          </Badge>
                        </div>
                    ...
                    <div className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <Button
                        ...
                        variant="ghost"
                        onClick={() =>
                          removeFromWatchlist.mutate(instrument.symbol)
                        }
              ...
              .map((instrument) => (
                ...
                >
                  <div className="flex items-start justify-between">
                    ...
                    <span
                      className={`text-xs font-mono font-bold ${instrument.pct >= 0 ? "text-gain" : "text-loss"}`}
                    >
                      {instrument.pct >= 0 ? "+" : ""}
                      {instrument.pct.toFixed(2)}%
                    </span>
                  </div>
                </button>
              ))}
